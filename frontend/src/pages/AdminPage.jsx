...
export default function AdminPage() {
  ...
  const handleCollectData = async () => {
    if (!window.confirm("데이터 수집을 시작하시겠습니까? (시간이 걸릴 수 있습니다)")) {
      return;
    }
    ...
    try {
      ...
      // LOST112 데이터 수집
      const { data } = await api.post("/admin/import/lost112", {
        maxPages: 5,
        rowsPerPage: 100
      });
      ...
      alert("데이터 수집에 실패했습니다: " + (err?.response?.data?.message || err.message));
    } finally {
    ...
    <div className="min-h-screen bg-gray-50 py-8 font-elice">
      <div className="max-w-7xl mx-auto px-4">
        ...
        <div className="bg-white rounded-lg shadow-sm p-6 mb-6">
          <h1 className="text-3xl font-bold text-[#3D3D3D] mb-2">관리자 페이지</h1>
              ...
              <>
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                  <div className="bg-white rounded-lg shadow-sm p-6">
                    <h3 className="text-lg font-semibold text-gray-700 mb-2">전체 사용자</h3>
                    <p className="text-3xl font-bold text-[#A2AADB]">
                      {stats.users?.total || 0}명
                    </p>
                    <p className="text-sm text-gray-600 mt-2">
                      관리자: {stats.users?.admin || 0}명 | 일반: {stats.users?.normal || 0}명
                    </p>
                  </div>

                  <div className="bg-white rounded-lg shadow-sm p-6">
                    <h3 className="text-lg font-semibold text-gray-700 mb-2">분실물 데이터</h3>
                    <p className="text-3xl font-bold text-[#A2AADB]">
                      {stats.lostItems?.total?.toLocaleString() || 0}건
                    </p>
                    <p className="text-sm text-gray-600 mt-2">
                      지역정보: {stats.lostItems?.withRegion?.toLocaleString() || 0}건
                    </p>
                  </div>

                  <div className="bg-white rounded-lg shadow-sm p-6">
                    <h3 className="text-lg font-semibold text-gray-700 mb-2">지역 커버리지</h3>
                    <p className="text-3xl font-bold text-[#A2AADB]">
                ...
                <div className="bg-white rounded-lg shadow-sm p-6">
                  ...
                  <div className="space-y-2 text-sm text-gray-600">
                    <p>마지막 업데이트: {stats.timestamp ? new Date(stats.timestamp).toLocaleString("ko-KR") : "-"}</p>
                    <p>시스템 상태: <span className="text-green-600 font-semibold">정상</span></p>
                  </div>
                </div>
              </>
            ) : (
        ...
        {activeTab === "users" && (
          <div className="bg-white rounded-lg shadow-sm p-6">
            <div className="flex items-center justify-between mb-6">
              <h3 className="text-xl font-semibold text-gray-800">사용자 목록</h3>
              <button
                onClick={fetchUsers}
                disabled={loading}
        ...
        {activeTab === "data" && (
          <div className="space-y-6">
            <div className="bg-white rounded-lg shadow-sm p-6">
              <h3 className="text-xl font-semibold text-gray-800 mb-4">데이터 수집</h3>
              ...
              <button
                ...
                className="w-full md:w-auto px-8 py-4 bg-[#A2AADB] text-white rounded-lg hover:bg-[#8B94C7] transition-colors disabled:opacity-50 disabled:cursor-not-allowed font-semibold text-lg"
              >
                {collectingData ? "데이터 수집 중..." : "데이터 수집 시작"}
              </button>

              {collectingData && (
                <div className="mt-6 p-4 bg-blue-50 rounded-lg">
                  <p className="text-blue-700">데이터를 수집하고 있습니다. 잠시만 기다려주세요...</p>
                </div>
              )}
            </div>
            ...
            {collectionResult && (
              <div className="bg-white rounded-lg shadow-sm p-6">
                <h3 className="text-xl font-semibold text-gray-800 mb-4">수집 결과</h3>
                <div className="space-y-3">
                  <div className="flex items-center justify-between p-3 bg-gray-50 rounded">
                    <span className="text-gray-700">수집 성공 여부</span>
                    <span className={`font-semibold ${collectionResult.success ? "text-green-600" : "text-red-600"}`}>
                  ...
                  {collectionResult.message && (
                    <div className="p-3 bg-blue-50 rounded">
                      <p className="text-sm text-blue-700">{collectionResult.message}</p>
                    </div>
                  )}
                </div>
              </div>
